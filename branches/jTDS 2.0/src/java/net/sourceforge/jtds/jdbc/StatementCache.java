 ...
 *
 * @version $Id: StatementCache.java,v 1.1 2007-09-10 19:19:32 bheineman Exp $
 */
class StatementCache {
    ...
    private static class CacheEntry {
         ...
         */
        CacheEntry(final String key, final ProcEntry value) {
            this.key = key;
            this.value = value;
        }
     ...
     */
    StatementCache(final int cacheSize) {
        this.cacheSize = cacheSize;
        cache = new HashMap<String, CacheEntry>(Math.min(MAX_INITIAL_SIZE, cacheSize) + 1);
        head  = new CacheEntry(null, null);
        tail  = new CacheEntry(null, null);
        head.next = tail;
        tail.prior = head;
        free = new ArrayList<ProcEntry>();
    }
     ...
     *         does not exist
     */
    synchronized Object get(final String key) {
     ...
     */
    synchronized void put(final String key, final Object handle) {
        // Increment usage count
     ...
     */
    synchronized void remove(final String key) {
        CacheEntry ce =  cache.get(key);
        if (ce != null) {
            // remove entry from linked list
            ce.unlink();
     ...
     */
    synchronized Collection getObsoleteHandles(final Collection handles) {
        if (handles != null) {
