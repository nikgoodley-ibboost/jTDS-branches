 ...
 * driver based on JDBC<sup><font size=-2>TM</font></sup> technology ("JDBC
 * driver") that is used with it. Different relational DBMSs often support
 * different features, implement features in different ways, and use different
 * data types. In addition, a driver may implement a feature on top of what the
 * DBMS offers. Information returned by methods in this interface applies to the
 * capabilities of a particular driver and a particular DBMS working together.
 ...
 * used in a CREATE TABLE statement. Or a user might call the method
 ...
 * <code>String</code> patterns. These arguments all have names such as
 * <code>fooPattern</code>. Within a pattern String, "%" means match any
 * substring of 0 or more characters, and "_" means match any one character.
 * Only metadata entries matching the search pattern are returned. If a search
 * pattern argument is set to <code>null</code>, that argument's criterion will
 * be dropped from the search.
 * <p/>
 * A method that gets information about a feature that the driver does not
 * support will throw an <code>SQLException</code>. In the case of methods that
 * return a <code>ResultSet</code> object, either a <code>ResultSet</code>
 * object (which may be empty) is returned or an <code>SQLException </code> is
 * thrown.
 */
public interface DatabaseMetaData {
     ...
     * database.  These are the  Open Group CLI string function names used in
     ...
     * database.  These are the  Open Group CLI system function names used in
     * the JDBC function escape clause.
     *
     ...
     * characters).
     * <p/>
     * <P>The '_' character represents any single character; the '%' character
     ...
     * @param toType   the type to convert to; one of the type codes from the
     *                 class <code>java.sql.Types</code>
     * @return <code>true</code> if so; <code>false</code> otherwise
     * @throws SQLException if a database access error occurs
     * @see Types
    ...
    /**
     * Retrieves whether this database supports expressions in <code>ORDER
     * BY</code> lists.
     *
     * @return <code>true</code> if so; <code>false</code> otherwise
     * @throws SQLException if a database access error occurs
    ...
    /**
     * Retrieves whether this database supports some form of <code>GROUP
     * BY</code> clause.
     *
     * @return <code>true</code> if so; <code>false</code> otherwise
     * @throws SQLException if a database access error occurs
     ...
     *  (This will be <code>true</code> if the method <code>supportsFullOuterJoins</code>
     ...
     * catalog. <P> Only procedure descriptions matching the schema and
     * procedure name criteria are returned.  They are ordered by
     * <code>PROCEDURE_SCHEM</code> and <code>PROCEDURE_NAME</code>.
     * <p/>
     * <P>Each procedure description has the the following columns: <OL>
     * <LI><B>PROCEDURE_CAT</B> String => procedure catalog (may be
     * <code>null</code>) <LI><B>PROCEDURE_SCHEM</B> String => procedure schema
     * (may be <code>null</code>) <LI><B>PROCEDURE_NAME</B> String => procedure
     * name <LI> reserved for future use <LI> reserved for future use <LI>
     * reserved for future use <LI><B>REMARKS</B> String => explanatory comment
     * on the procedure <LI><B>PROCEDURE_TYPE</B> short => kind of procedure:
     * <UL> <LI> procedureResultUnknown - May return a result <LI>
     * procedureNoResult - Does not return a result <LI> procedureReturnsResult
     * - Returns a result </UL> </OL>
     *
     * @param catalog              a catalog name; must match the catalog name
     *                             as it is stored in the database; "" retrieves
     *                             those without a catalog; <code>null</code>
     ...
     * <P> A possible value for column <code>PROCEDURE_TYPE</code> in the
    ...
    /**
     * Indicates that the procedure does not return a result. <P> A possible
     * value for column <code>PROCEDURE_TYPE</code> in the
     * <code>ResultSet</code> object returned by the method
     * <code>getProcedures</code>.
     */
    ...
    /**
     * Indicates that the procedure returns a result. <P> A possible value for
     * column <code>PROCEDURE_TYPE</code> in the <code>ResultSet</code> object
     ...
     * column description with the following fields: <OL>
     * <LI><B>PROCEDURE_CAT</B> String => procedure catalog (may be
     * <code>null</code>) <LI><B>PROCEDURE_SCHEM</B> String => procedure schema
     * (may be <code>null</code>) <LI><B>PROCEDURE_NAME</B> String => procedure
     * name <LI><B>COLUMN_NAME</B> String => column/parameter name
     * <LI><B>COLUMN_TYPE</B> Short => kind of column/parameter: <UL> <LI>
     * procedureColumnUnknown - nobody knows <LI> procedureColumnIn - IN
     * parameter <LI> procedureColumnInOut - INOUT parameter <LI>
     * procedureColumnOut - OUT parameter <LI> procedureColumnReturn - procedure
     * return value <LI> procedureColumnResult - result column in
     * <code>ResultSet</code> </UL> <LI><B>DATA_TYPE</B> int => SQL type from
     * java.sql.Types <LI><B>TYPE_NAME</B> String => SQL type name, for a UDT
     * type the type name is fully qualified <LI><B>PRECISION</B> int =>
     * precision <LI><B>LENGTH</B> int => length in bytes of data
     * <LI><B>SCALE</B> short => scale <LI><B>RADIX</B> short => radix
     * <LI><B>NULLABLE</B> short => can it contain NULL. <UL> <LI>
     * procedureNoNulls - does not allow NULL values <LI> procedureNullable -
     * allows NULL values <LI> procedureNullableUnknown - nullability unknown
     * </UL> <LI><B>REMARKS</B> String => comment describing parameter/column
     * </OL>
     * <p/>
    ...
    /**
     * Indicates that type of the column is unknown. <P> A possible value for
     * the column <code>COLUMN_TYPE</code> in the <code>ResultSet</code>
     * returned by the method <code>getProcedureColumns</code>.
     */
    int procedureColumnUnknown = 0;

    /**
     * Indicates that the column stores IN parameters. <P> A possible value for
     * the column <code>COLUMN_TYPE</code> in the <code>ResultSet</code>
     * returned by the method <code>getProcedureColumns</code>.
     */
    ...
    /**
     * Indicates that the column stores INOUT parameters. <P> A possible value
     * for the column <code>COLUMN_TYPE</code> in the <code>ResultSet</code>
     * returned by the method <code>getProcedureColumns</code>.
     */
    int procedureColumnInOut = 2;

    /**
     * Indicates that the column stores OUT parameters. <P> A possible value for
     * the column <code>COLUMN_TYPE</code> in the <code>ResultSet</code>
     * returned by the method <code>getProcedureColumns</code>.
     */
    int procedureColumnOut = 4;
    /**
     * Indicates that the column stores return values. <P> A possible value for
     * the column <code>COLUMN_TYPE</code> in the <code>ResultSet</code>
     * returned by the method <code>getProcedureColumns</code>.
     */
    ...
    /**
     * Indicates that the column stores results. <P> A possible value for the
     * column <code>COLUMN_TYPE</code> in the <code>ResultSet</code> returned by
     * the method <code>getProcedureColumns</code>.
     */
    int procedureColumnResult = 3;

    /**
     * Indicates that <code>NULL</code> values are not allowed. <P> A possible
     * value for the column <code>NULLABLE</code> in the <code>ResultSet</code>
     * object returned by the method <code>getProcedureColumns</code>.
     */
    int procedureNoNulls = 0;

    /**
     * Indicates that <code>NULL</code> values are allowed. <P> A possible value
     * for the column <code>NULLABLE</code> in the <code>ResultSet</code> object
     * returned by the method <code>getProcedureColumns</code>.
     */
     ...
     * <P> A possible value for the column <code>NULLABLE</code> in the
     * <code>ResultSet</code> object returned by the method
     ...
     * TABLE_NAME. <P> Each table description has the following columns: <OL>
     ...
     * <code>null</code>) <LI><B>TYPE_SCHEM</B> String => the types schema (may
     * be <code>null</code>) <LI><B>TYPE_NAME</B> String => type name (may be
     * <code>null</code>) <LI><B>SELF_REFERENCING_COL_NAME</B> String => name of
     * the designated "identifier" column of a typed table (may be
     * <code>null</code>) <LI><B>REF_GENERATION</B> String => specifies how
     * values in SELF_REFERENCING_COL_NAME are created. Values are "SYSTEM",
     ...
     * name <LI><B>TABLE_CATALOG</B> String => catalog name (may be
     * <code>null</code>) </OL>
     *
     ...
     * => column name <LI><B>DATA_TYPE</B> int => SQL type from java.sql.Types
     * <LI><B>TYPE_NAME</B> String => Data source dependent type name, for a UDT
     * the type name is fully qualified <LI><B>COLUMN_SIZE</B> int => column
     ...
     * digits <LI><B>NUM_PREC_RADIX</B> int => Radix (typically either 10 or 2)
     * <LI><B>NULLABLE</B> int => is NULL allowed. <UL> <LI> columnNoNulls -
     * might not allow <code>NULL</code> values <LI> columnNullable - definitely
     * allows <code>NULL</code> values <LI> columnNullableUnknown - nullability
     * unknown </UL> <LI><B>REMARKS</B> String => comment describing column (may
     * be <code>null</code>) <LI><B>COLUMN_DEF</B> String => default value (may
     * be <code>null</code>) <LI><B>SQL_DATA_TYPE</B> int => unused
     * <LI><B>SQL_DATETIME_SUB</B> int => unused <LI><B>CHAR_OCTET_LENGTH</B>
     ...
     * 1) <LI><B>IS_NULLABLE</B> String => "NO" means column definitely does not
     * allow NULL values; "YES" means the column might allow NULL values.  An
     * empty string means nobody knows. <LI><B>SCOPE_CATLOG</B> String =>
     * catalog of table that is the scope of a reference attribute
     * (<code>null</code> if DATA_TYPE isn't REF) <LI><B>SCOPE_SCHEMA</B> String
     * => schema of table that is the scope of a reference attribute
     * (<code>null</code> if the DATA_TYPE isn't REF) <LI><B>SCOPE_TABLE</B>
     * String => table name that this the scope of a reference attribure
     * (<code>null</code> if the DATA_TYPE isn't REF) <LI><B>SOURCE_DATA_TYPE</B>
    ...
    /**
     * Indicates that the column might not allow <code>NULL</code> values. <P> A
     * possible value for the column <code>NULLABLE</code> in the
     * <code>ResultSet</code> returned by the method <code>getColumns</code>.
     */
    int columnNoNulls = 0;

    /**
     * Indicates that the column definitely allows <code>NULL</code> values. <P>
    ...
    /**
     * Indicates that the nullability of columns is unknown. <P> A possible
     * value for the column <code>NULLABLE</code> in the <code>ResultSet</code>
     * returned by the method <code>getColumns</code>.
     */
     ...
     * <p/>
     * <P>Each privilige description has the following columns: <OL>
     * <LI><B>TABLE_CAT</B> String => table catalog (may be <code>null</code>)
     * <LI><B>TABLE_SCHEM</B> String => table schema (may be <code>null</code>)
     * <LI><B>TABLE_NAME</B> String => table name <LI><B>COLUMN_NAME</B> String
     * => column name <LI><B>GRANTOR</B> => grantor of access (may be
     * <code>null</code>) <LI><B>GRANTEE</B> String => grantee of access
     * <LI><B>PRIVILEGE</B> String => name of access (SELECT, INSERT, UPDATE,
     * REFRENCES, ...) <LI><B>IS_GRANTABLE</B> String => "YES" if grantee is
     * permitted to grant to others; "NO" if not; <code>null</code> if unknown
     * </OL>
     *
     * @param catalog           a catalog name; must match the catalog name as
     *                          it is stored in the database; "" retrieves those
     ...
     *                          is stored in the database; "" retrieves those
     *                          without a schema; <code>null</code> means that
     *                          the schema name should not be used to narrow the
     *                          search
     ...
     * <LI><B>TABLE_CAT</B> String => table catalog (may be <code>null</code>)
     * <LI><B>TABLE_SCHEM</B> String => table schema (may be <code>null</code>)
     * <LI><B>TABLE_NAME</B> String => table name <LI><B>GRANTOR</B> => grantor
     * of access (may be <code>null</code>) <LI><B>GRANTEE</B> String => grantee
     * of access <LI><B>PRIVILEGE</B> String => name of access (SELECT, INSERT,
     * UPDATE, REFRENCES, ...) <LI><B>IS_GRANTABLE</B> String => "YES" if
     * grantee is permitted to grant to others; "NO" if not; <code>null</code>
     * if unknown </OL>
     *
     * @param catalog          a catalog name; must match the catalog name as it
     *                         is stored in the database; "" retrieves those
     ...
     * <p/>
     * <P>Each column description has the following columns: <OL>
     * <LI><B>SCOPE</B> short => actual scope of result <UL> <LI>
     * bestRowTemporary - very temporary, while using row <LI>
     * bestRowTransaction - valid for remainder of current transaction <LI>
     * bestRowSession - valid for remainder of current session </UL>
     * <LI><B>COLUMN_NAME</B> String => column name <LI><B>DATA_TYPE</B> int =>
     ...
     * <LI><B>COLUMN_SIZE</B> int => precision <LI><B>BUFFER_LENGTH</B> int =>
     * not used <LI><B>DECIMAL_DIGITS</B> short	 => scale
     * <LI><B>PSEUDO_COLUMN</B> short => is this a pseudo column like an Oracle
     * ROWID <UL> <LI> bestRowUnknown - may or may not be pseudo column <LI>
     * bestRowNotPseudo - is NOT a pseudo column <LI> bestRowPseudo - is a
     * pseudo column </UL> </OL>
     *
     ...
     * lasting only while the row is being used. <P> A possible value for the
     * column <code>SCOPE</code> in the <code>ResultSet</code> object returned
     * by the method <code>getBestRowIdentifier</code>.
     */
    int bestRowTemporary = 0;

    /**
     * Indicates that the scope of the best row identifier is the remainder of
     * the current transaction. <P> A possible value for the column
     * <code>SCOPE</code> in the <code>ResultSet</code> object returned by the
     * method <code>getBestRowIdentifier</code>.
     */
    int bestRowTransaction = 1;

    /**
     * Indicates that the scope of the best row identifier is the remainder of
     * the current session. <P> A possible value for the column
     * <code>SCOPE</code> in the <code>ResultSet</code> object returned by the
     * method <code>getBestRowIdentifier</code>.
     */
     ...
     * <P> A possible value for the column <code>PSEUDO_COLUMN</code> in the
     * <code>ResultSet</code> object returned by the method
    ...
    /**
     * Indicates that the best row identifier is NOT a pseudo column. <P> A
     * possible value for the column <code>PSEUDO_COLUMN</code> in the
     * <code>ResultSet</code> object returned by the method
     * <code>getBestRowIdentifier</code>.
     */
    ...
    /**
     * Indicates that the best row identifier is a pseudo column. <P> A possible
     * value for the column <code>PSEUDO_COLUMN</code> in the
     * <code>ResultSet</code> object returned by the method
     * <code>getBestRowIdentifier</code>.
     */
    int bestRowPseudo = 2;
     ...
     * <p/>
     * <P>Each column description has the following columns: <OL>
     * <LI><B>SCOPE</B> short => is not used <LI><B>COLUMN_NAME</B> String =>
     * column name <LI><B>DATA_TYPE</B> int => SQL data type from
     * <code>java.sql.Types</code> <LI><B>TYPE_NAME</B> String => Data
     ...
     * => whether this is pseudo column like an Oracle ROWID <UL> <LI>
     * versionColumnUnknown - may or may not be pseudo column <LI>
     * versionColumnNotPseudo - is NOT a pseudo column <LI> versionColumnPseudo
    ...
    /**
     * Indicates that this version column may or may not be a pseudo column. <P>
    ...
    /**
     * Indicates that this version column is NOT a pseudo column. <P> A possible
     * value for the column <code>PSEUDO_COLUMN</code> in the
     * <code>ResultSet</code> object returned by the method
     * <code>getVersionColumns</code>.
     */
    ...
    /**
     * Indicates that this version column is a pseudo column. <P> A possible
     * value for the column <code>PSEUDO_COLUMN</code> in the
     * <code>ResultSet</code> object returned by the method
     * <code>getVersionColumns</code>.
     */
     ...
     * <LI><B>TABLE_SCHEM</B> String => table schema (may be <code>null</code>)
     * <LI><B>TABLE_NAME</B> String => table name <LI><B>COLUMN_NAME</B> String
     * => column name <LI><B>KEY_SEQ</B> short => sequence number within primary
     * key <LI><B>PK_NAME</B> String => primary key name (may be
     * <code>null</code>) </OL>
     *
     * @param catalog a catalog name; must match the catalog name as it is
     *                stored in the database; "" retrieves those without a
     ...
     * (may be <code>null</code>) <LI><B>PKTABLE_SCHEM</B> String => primary key
     * table schema being imported (may be <code>null</code>)
     ...
     * <code>null</code>) <LI><B>FKTABLE_SCHEM</B> String => foreign key table
     * schema (may be <code>null</code>) <LI><B>FKTABLE_NAME</B> String =>
     * foreign key table name <LI><B>FKCOLUMN_NAME</B> String => foreign key
     * column name <LI><B>KEY_SEQ</B> short => sequence number within a foreign
     * key <LI><B>UPDATE_RULE</B> short => What happens to a foreign key when
     * the primary key is updated: <UL> <LI> importedNoAction - do not allow
     * update of primary key if it has been imported <LI> importedKeyCascade -
     * change imported key to agree with primary key update <LI>
     * importedKeySetNull - change imported key to <code>NULL</code> if its
     * primary key has been updated <LI> importedKeySetDefault - change imported
     * key to default values if its primary key has been updated <LI>
     * importedKeyRestrict - same as importedKeyNoAction (for ODBC 2.x
     * compatibility) </UL> <LI><B>DELETE_RULE</B> short => What happens to the
     * foreign key when primary is deleted. <UL> <LI> importedKeyNoAction - do
     * not allow delete of primary key if it has been imported <LI>
     ...
     * String => foreign key name (may be <code>null</code>) <LI><B>PK_NAME</B>
     * String => primary key name (may be <code>null</code>)
     * <LI><B>DEFERRABILITY</B> short => can the evaluation of foreign key
     * constraints be deferred until commit <UL> <LI> importedKeyInitiallyDeferred
     * - see SQL92 for definition <LI> importedKeyInitiallyImmediate - see SQL92
     * for definition <LI> importedKeyNotDeferrable - see SQL92 for definition
     * </UL> </OL>
     *
     * @param catalog a catalog name; must match the catalog name as it is
     *                stored in the database; "" retrieves those without a
     ...
     * primary key is deleted, rows that imported that key are deleted. <P> A
     * possible value for the columns <code>UPDATE_RULE</code> and
     ...
     * <P> A possible value for the columns <code>UPDATE_RULE</code> and
     * <code>DELETE_RULE</code> in the <code>ResultSet</code> objects returned
     * by the methods <code>getImportedKeys</code>,  <code>getExportedKeys</code>,
     * and <code>getCrossReference</code>.
     */
     ...
     * key (imported key) is changed to <code>NULL</code>. <P> A possible value
     * for the columns <code>UPDATE_RULE</code> and <code>DELETE_RULE</code> in
     * the <code>ResultSet</code> objects returned by the methods
     ...
     * or deleted. <P> A possible value for the columns <code>UPDATE_RULE</code>
     * and <code>DELETE_RULE</code> in the <code>ResultSet</code> objects
     * returned by the methods <code>getImportedKeys</code>,
     ...
     * (imported key) is set to the default value. <P> A possible value for the
     * columns <code>UPDATE_RULE</code> and <code>DELETE_RULE</code> in the
    ...
    /**
     * Indicates deferrability.  See SQL-92 for a definition. <P> A possible
     * value for the column <code>DEFERRABILITY</code> in the
     * <code>ResultSet</code> objects returned by the methods
     * <code>getImportedKeys</code>,  <code>getExportedKeys</code>, and
     * <code>getCrossReference</code>.
     */
     ...
     * <code>null</code>) <LI><B>PKTABLE_SCHEM</B> String => primary key table
     * schema (may be <code>null</code>) <LI><B>PKTABLE_NAME</B> String =>
     * primary key table name <LI><B>PKCOLUMN_NAME</B> String => primary key
     * column name <LI><B>FKTABLE_CAT</B> String => foreign key table catalog
     * (may be <code>null</code>) being exported (may be <code>null</code>)
     ...
     * is updated: <UL> <LI> importedNoAction - do not allow update of primary
     * key if it has been imported <LI> importedKeyCascade - change imported key
     * to agree with primary key update <LI> importedKeySetNull - change
     * imported key to <code>NULL</code> if its primary key has been updated
     ...
     * importedKeyNoAction (for ODBC 2.x compatibility) </UL>
     * <LI><B>DELETE_RULE</B> short => What happens to the foreign key when
     * primary is deleted. <UL> <LI> importedKeyNoAction - do not allow delete
     * of primary key if it has been imported <LI> importedKeyCascade - delete
     * rows that import a deleted key <LI> importedKeySetNull - change imported
     * key to <code>NULL</code> if its primary key has been deleted <LI>
     * importedKeyRestrict - same as importedKeyNoAction (for ODBC 2.x
     * compatibility) <LI> importedKeySetDefault - change imported key to
     * default if its primary key has been deleted </UL> <LI><B>FK_NAME</B>
     * String => foreign key name (may be <code>null</code>) <LI><B>PK_NAME</B>
     * String => primary key name (may be <code>null</code>)
     * <LI><B>DEFERRABILITY</B> short => can the evaluation of foreign key
     * constraints be deferred until commit <UL> <LI> importedKeyInitiallyDeferred
     ...
     * </UL> </OL>
     *
     * @param primaryCatalog a catalog name; must match the catalog name as it
     *                       is stored in the database; "" retrieves those
     *                       without a catalog; <code>null</code> means drop
     ...
     *                       a schema; <code>null</code> means drop schema name
     *                       from the selection criteria
     * @param primaryTable   the name of the table that exports the key; must
     *                       match the table name as it is stored in the
     *                       database
     * @param foreignCatalog a catalog name; must match the catalog name as it
     *                       is stored in the database; "" retrieves those
     *                       without a catalog; <code>null</code> means drop
     *                       catalog name from the selection criteria
     ...
     * <p/>
     * <P>Each type description has the following columns: <OL>
     * <LI><B>TYPE_NAME</B> String => Type name <LI><B>DATA_TYPE</B> int => SQL
     * data type from java.sql.Types <LI><B>PRECISION</B> int => maximum
     * precision <LI><B>LITERAL_PREFIX</B> String => prefix used to quote a
     * literal (may be <code>null</code>) <LI><B>LITERAL_SUFFIX</B> String =>
     * suffix used to quote a literal (may be <code>null</code>)
     * <LI><B>CREATE_PARAMS</B> String => parameters used in creating the type
     * (may be <code>null</code>) <LI><B>NULLABLE</B> short => can you use NULL
     * for this type. <UL> <LI> typeNoNulls - does not allow NULL values <LI>
     * typeNullable - allows NULL values <LI> typeNullableUnknown - nullability
     * unknown </UL> <LI><B>CASE_SENSITIVE</B> boolean=> is it case sensitive.
     * <LI><B>SEARCHABLE</B> short => can you use "WHERE" based on this type:
     * <UL> <LI> typePredNone - No support <LI> typePredChar - Only supported
     * with WHERE .. LIKE <LI> typePredBasic - Supported except for WHERE ..
     * LIKE <LI> typeSearchable - Supported for all WHERE .. </UL>
     * <LI><B>UNSIGNED_ATTRIBUTE</B> boolean => is it unsigned.
     * <LI><B>FIXED_PREC_SCALE</B> boolean => can it be a money value.
     * <LI><B>AUTO_INCREMENT</B> boolean => can it be used for an auto-increment
     * value. <LI><B>LOCAL_TYPE_NAME</B> String => localized version of type
     * name (may be <code>null</code>) <LI><B>MINIMUM_SCALE</B> short => minimum
     * scale supported <LI><B>MAXIMUM_SCALE</B> short => maximum scale supported
     ...
     * type. <P> A possible value for column <code>NULLABLE</code> in the
     * <code>ResultSet</code> object returned by the method
     ...
     * <P> A possible value for column <code>NULLABLE</code> in the
     * <code>ResultSet</code> object returned by the method
     * <code>getTypeInfo</code>.
     */
    int typeNullable = 1;
     ...
     * allowed for this data type. <P> A possible value for column
     * <code>NULLABLE</code> in the <code>ResultSet</code> object returned by
     * the method <code>getTypeInfo</code>.
     */
    int typeNullableUnknown = 2;

    /**
     * Indicates that <code>WHERE</code> search clauses are not supported for
     * this type. <P> A possible value for column <code>SEARCHABLE</code> in the
     * <code>ResultSet</code> object returned by the method
     * <code>getTypeInfo</code>.
     */
    int typePredNone = 0;
    ...
    /**
     * Indicates that the only <code>WHERE</code> search clause that can be
     * based on this type is <code>WHERE . . . LIKE</code>. <P> A possible value
     * for column <code>SEARCHABLE</code> in the <code>ResultSet</code> object
     * returned by the method <code>getTypeInfo</code>.
     */
    int typePredChar = 1;

    /**
     * Indicates that one can base all <code>WHERE</code> search clauses except
     * <code>WHERE . . . LIKE</code> on this data type. <P> A possible value for
     * column <code>SEARCHABLE</code> in the <code>ResultSet</code> object
     * returned by the method <code>getTypeInfo</code>.
     */
    int typePredBasic = 2;
     ...
     * type. <P> A possible value for column <code>SEARCHABLE</code> in the
     * <code>ResultSet</code> object returned by the method
     * <code>getTypeInfo</code>.
     */
     ...
     * <p/>
     * <P>Each index column description has the following columns: <OL>
     * <LI><B>TABLE_CAT</B> String => table catalog (may be <code>null</code>)
     * <LI><B>TABLE_SCHEM</B> String => table schema (may be <code>null</code>)
     * <LI><B>TABLE_NAME</B> String => table name <LI><B>NON_UNIQUE</B> boolean
     ...
     * <LI><B>INDEX_NAME</B> String => index name; <code>null</code> when TYPE
     * is tableIndexStatistic <LI><B>TYPE</B> short => index type: <UL> <LI>
     * tableIndexStatistic - this identifies table statistics that are returned
     * in conjuction with a table's index descriptions <LI> tableIndexClustered
     * - this is a clustered index <LI> tableIndexHashed - this is a hashed
     * index <LI> tableIndexOther - this is some other style of index </UL>
     ...
     * column name; <code>null</code> when TYPE is tableIndexStatistic
     * <LI><B>ASC_OR_DESC</B> String => column sort sequence, "A" => ascending,
     * "D" => descending, may be <code>null</code> if sort sequence is not
     * supported; <code>null</code> when TYPE is tableIndexStatistic
     ...
     *                    false, return indices regardless of whether unique or
     *                    not
     * @param approximate when true, result is allowed to reflect approximate or
     *                    out of data values; when false, results are requested
     *                    to be accurate
     ...
     * conjunction with a table's index descriptions. <P> A possible value for
     * column <code>TYPE</code> in the <code>ResultSet</code> object returned by
     * the method <code>getIndexInfo</code>.
     */
    short tableIndexStatistic = 0;

    /**
     * Indicates that this table index is a clustered index. <P> A possible
     * value for column <code>TYPE</code> in the <code>ResultSet</code> object
     * returned by the method <code>getIndexInfo</code>.
     */
    short tableIndexClustered = 1;

    /**
     * Indicates that this table index is a hashed index. <P> A possible value
     * for column <code>TYPE</code> in the <code>ResultSet</code> object
     * returned by the method <code>getIndexInfo</code>.
     */
     ...
     * or table statistics; it is something other than these. <P> A possible
     * value for column <code>TYPE</code> in the <code>ResultSet</code> object
     * returned by the method <code>getIndexInfo</code>.
     */
     ...
     * <code>null</code>) <LI><B>TYPE_SCHEM</B> String => type's schema (may be
     * <code>null</code>) <LI><B>TYPE_NAME</B> String => type name
     * <LI><B>CLASS_NAME</B> String => Java class name <LI><B>DATA_TYPE</B> int
     * => type value defined in java.sql.Types. One of JAVA_OBJECT, STRUCT, or
     * DISTINCT <LI><B>REMARKS</B> String => explanatory comment on the type
     ...
     * REFERENCE_GENERATION = USER_DEFINED) </OL>
     * <p/>
     * <P><B>Note:</B> If the driver does not support UDTs, an empty result set
     * is returned.
     *
     * @param catalog         a catalog name; must match the catalog name as it
     *                        is stored in the database; "" retrieves those
     ...
     *                        it is stored in the database; may be a fully
     *                        qualified name
     * @param types           a list of user-defined types (JAVA_OBJECT, STRUCT,
     *                        or DISTINCT) to include; <code>null</code> returns
     *                        all types
     ...
     * type/ sub type relationship is modeled. <P> Only supertype information
     * for UDTs matching the catalog, schema, and type name is returned. The
     * type name parameter may be a fully-qualified name. When the UDT name
     * supplied is a fully-qualified name, the catalog and schemaPattern
     * parameters are ignored. <P> If a UDT does not have a direct super type,
     * it is not listed here. A row of the <code>ResultSet</code> object
     * returned by this method describes the designated UDT and a direct
     * supertype. A row has the following columns: <OL> <LI><B>TYPE_CAT</B>
     * String => the UDT's catalog (may be <code>null</code>)
     * <LI><B>TYPE_SCHEM</B> String => UDT's schema (may be <code>null</code>)
     * <LI><B>TYPE_NAME</B> String => type name of the UDT
     * <LI><B>SUPERTYPE_CAT</B> String => the direct super type's catalog (may
     * be <code>null</code>) <LI><B>SUPERTYPE_SCHEM</B> String => the direct
     * super type's schema (may be <code>null</code>) <LI><B>SUPERTYPE_NAME</B>
     * String => the direct super type's name </OL>
     * <p/>
     * <P><B>Note:</B> If the driver does not support type hierarchies, an empty
     ...
     * <code>null</code>) <LI><B>TABLE_SCHEM</B> String => type's schema (may be
     * <code>null</code>) <LI><B>TABLE_NAME</B> String => type name
     * <LI><B>SUPERTABLE_NAME</B> String => the direct super type's name </OL>
     * <p/>
     * <P><B>Note:</B> If the driver does not support type hierarchies, an empty
     * result set is returned.
     *
     * @param catalog          a catalog name; "" retrieves those without a
     *                         catalog; <code>null</code> means drop catalog
    ...
    /**
     * Indicates that <code>NULL</code> values might not be allowed. <P> A
     * possible value for the column <code>NULLABLE</code> in the
     * <code>ResultSet</code> object returned by the method
    ...
    /**
     * Indicates that <code>NULL</code> values are definitely allowed. <P> A
     * possible value for the column <code>NULLABLE</code> in the
     * <code>ResultSet</code> object returned by the method
     * <code>getAttributes</code>.
     */
     ...
     * catalog. <P> Descriptions are returned only for attributes of UDTs
     * matching the catalog, schema, type, and attribute name criteria. They are
     * ordered by TYPE_SCHEM, TYPE_NAME and ORDINAL_POSITION. This description
     * does not contain inherited attributes. <P> The <code>ResultSet</code>
     * object that is returned has the following columns: <OL>
     * <LI><B>TYPE_CAT</B> String => type catalog (may be <code>null</code>)
     * <LI><B>TYPE_SCHEM</B> String => type schema (may be <code>null</code>)
     * <LI><B>TYPE_NAME</B> String => type name <LI><B>ATTR_NAME</B> String =>
     * attribute name <LI><B>DATA_TYPE</B> int => attribute type SQL type from
     * java.sql.Types <LI><B>ATTR_TYPE_NAME</B> String => Data source dependent
     * type name. For a UDT, the type name is fully qualified. For a REF, the
     * type name is fully qualified and represents the target type of the
     * reference type. <LI><B>ATTR_SIZE</B> int => column size.  For char or
     * date types this is the maximum number of characters; for numeric or
     * decimal types this is precision. <LI><B>DECIMAL_DIGITS</B> int => the
     * number of fractional digits <LI><B>NUM_PREC_RADIX</B> int => Radix
     * (typically either 10 or 2) <LI><B>NULLABLE</B> int => whether NULL is
     * allowed <UL> <LI> attributeNoNulls - might not allow NULL values <LI>
     * attributeNullable - definitely allows NULL values <LI>
     * attributeNullableUnknown - nullability unknown </UL> <LI><B>REMARKS</B>
     * String => comment describing column (may be <code>null</code>)
     * <LI><B>ATTR_DEF</B> String => default value (may be <code>null</code>)
     * <LI><B>SQL_DATA_TYPE</B> int => unused <LI><B>SQL_DATETIME_SUB</B> int =>
     * unused <LI><B>CHAR_OCTET_LENGTH</B> int => for char types the maximum
     * number of bytes in the column <LI><B>ORDINAL_POSITION</B> int	=> index of
     * column in table (starting at 1) <LI><B>IS_NULLABLE</B> String => "NO"
     * means column definitely does not allow NULL values; "YES" means the
     * column might allow NULL values.  An empty string means unknown.
     ...
     *                             attribute name as it is declared in the
     *                             database
     ...
     *
     * @return the default holdability; either <code>ResultSet.HOLD_CURSORS_OVER_COMMIT</code>
     *         or <code>ResultSet.CLOSE_CURSORS_AT_COMMIT</code>
     * @throws SQLException if a database access error occurs
     * @since 1.4
     */
    ...
    /**
     * Indicates that the value is an X/Open (now know as Open Group) SQL CLI
     * SQLSTATE value. <P> A possible return value for the method
    ...
    /**
     * Indicates that the value is an SQL99 SQLSTATE value. <P> A possible
     * return value for the method <code>SQLException.getSQLState</code>.
     *
     * @since 1.4
     */
    int sqlStateSQL99 = 2;
