 ...
 *
 * @version $Id: DatabaseMetaDataTest.java,v 1.1 2007-09-10 19:19:35 bheineman Exp $
 */
public class DatabaseMetaDataTest extends TestBase {
    ...
    public void testBooleanOptions() throws Exception {
        ...
        if (isSybase) {
            if (isSyb12) {
                ...
                assertFalse("supportsFullOuterJoins", dbmd.supportsFullOuterJoins());
            }
    ...
    public void testIntOptions() throws Exception {
        DatabaseMetaData dbmd = con.getMetaData();
        int sysnamelen = (isSql70 || isSql2K)? 128: 30;
        if (isSyb15) {
            sysnamelen = 255;
        }
    ...
    public void testResultSets() throws Exception
    {
        try {
            ...
            stmt.execute("CREATE TABLE jTDS_META (id int NOT NULL primary key , data varchar(255) NULL, ts timestamp)");
            ...
            boolean fail = true;
            while (rs.next()) {
                if (rs.getString(1).equalsIgnoreCase(con.getCatalog())) {
                    fail=false;
                    break;
                }
            }
            ...
            if (!isASA) {
                // Sybase ASA cannot return column privileges
                assertTrue(rs.next());
            ...
            if (!isASA) {
                // Sybase ASA cannot return columns
                assertTrue(rs.next());
            ...
            if (!isASA) {
                // Sybase ASA cannot return index info
                assertTrue(rs.next());
            ...
            //
            if (isASA) {
                rs = dbmd.getTables(con.getCatalog(), "%", "jTDS_META", new String[]{"%"});
            } else {
    ...
    public void testTableMetaData() throws Exception {
        ...
        ResultSet rs = stmt.executeQuery("SELECT name FROM master.dbo.sysdatabases");
    ...
    public void testColumnClassName() throws SQLException {
        ...
        String tabdef = "CREATE TABLE #testColumnClassName("
                ...
                + "colShort SMALLINT,"
                + "colInt INTEGER,"
                + "colBigint DECIMAL(28,0),"
                + "colFloat REAL,"
                + "colDouble FLOAT,"
                + "colDecimal DECIMAL(28,10),"
                + "colBit BIT,"
    ...
    public void testColumnMetaData() throws Exception {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String typeNameA = rs.getString("TYPE_NAME");
                String typeNameB = rsmd.getColumnTypeName(ord);
                if (isSybase) {
                    // Sybase catalog procedure returns varbinary for timestamp
                    // and varchar for sysname and char for nchar and varchar for nvarchar!
                    if (!(typeNameA.equals("varbinary") && typeNameB.equals("timestamp")) &&
                    !(typeNameA.equals("varchar") && typeNameB.equals("sysname")) &&
                    !(typeNameA.equals("char") && typeNameB.equals("nchar")) &&
                    !(typeNameA.equals("varchar") && typeNameB.equals("nvarchar"))) {
                        assertEquals(cn+" typename", rs.getString("TYPE_NAME"), rsmd.getColumnTypeName(ord));
                    }
                } else {
                    assertEquals(cn+" typename", rs.getString("TYPE_NAME"), rsmd.getColumnTypeName(ord));
                }
                if (!(isSql65 && typeNameA.equals("timestamp"))) {
                    assertEquals(cn+" datatype", rs.getInt("DATA_TYPE"), rsmd.getColumnType(ord));
                }
                int jdbcType = rs.getInt("DATA_TYPE");
                if (jdbcType != Types.REAL && jdbcType != Types.DOUBLE && jdbcType != Types.TIME) {
                ...
                // Sybase cat proc TIME returns wrong precision
                if (!(isSybase && jdbcType == Types.TIME)) {
    ...
    public void testProcedureColumns() throws Exception {
        try {
            ...
            if (!isASA && !isSql2005) {
                // ASA does not have a row for the return value
                // MS SQL2005 CTP June does not have a return value!
                assertTrue(rs.next());
            ...
            if (!isASA && !isSyb11) {
                // ASA does not tell us the column type
    ...
    public void testOrderedByDatatype() throws Exception {
        ResultSet typeInfoRs = con.getMetaData().getTypeInfo();
    ...
    public void testOrderedByTypeMapping() throws Exception {
        ResultSet typeInfoRs = con.getMetaData().getTypeInfo();
        List typeNames = getTypeNamesInOrder(typeInfoRs);
    ...
    private List getTypeNamesInOrder(ResultSet typeInfoRs) throws SQLException {
