...
public final class TdsStream {
    ...
    void setCharset(Charset cs) 
    {
        ...
        if (cs.newEncoder().maxBytesPerChar() > 1.0) {
            // Only worth doing for single byte character sets
            return;
        }
            ...
            if (maps == null) {
                ...
                try {
                    ...
                    if (charMap.length != MAX_BYTE_INDEX) {
                        return; // Should 256 if really max 1 byte per char!
                    }
    ...
    void write(char chars[], int len, Charset info) throws IOException {
        if (info.name().equals(charsetName)) {
            // OK Use our highspeed converter
    ...
    private void putPacket(int last) throws IOException {
        ...
        outBuffer[0] = pktType;
        outBuffer[1] = (byte) last; // last segment indicator
    ...
    String readString(int len, Charset info) throws IOException {
        ...
        read(bytes, 0, len);
        if (info.name().equals(charsetName)) {
            // OK we have a high speed mapping for this one
            char[] chars = (len > charBuffer.length)? new char[len]: charBuffer;
            for (int i = 0; i < len; i++) {
        ...
        //
        // Fall back on the full (but slow) Charset decoder.
        //
    ...
    String readUnicode(int len) throws IOException {
        ...
        while (dest < len) {
            ...
            if (available == 1) {
                // A unicode char might be split over two packets.
                int b1 = read();
            ...
            while (dest < limit) {
                // High speed copy using local variables
    ...
    int readInt() throws IOException {
        if (inBufferLen - inBufferPtr > 3) {
            ...
            return (inBuffer[inBufferPtr++] << 24) 
                     | (b3 << 16) | (b2 << 8) | b1;
        }
        int b1 = read();
        int b2 = read() << 8;
     ...
     */
    DateTime readDatetime(final int len)
            throws IOException {
        int daysSince1900;
        int time;
        int minutes;
        ...
        switch (len) {
            ...
            default:
                throw new IOException("Invalid DATETIME value with size of "
                                            + len + " bytes.");
        }
    }
    ...
    void skipToEnd() {
        try {
            ...
            inBufferPtr = inBufferLen;
            // Now consume all data until we get the last buffer
            while (inBuffer[1] != 1) {
