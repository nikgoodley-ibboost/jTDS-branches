...
public class BlobBuffer implements Serializable {
     ...
     */
    transient private RandomAccessFile raFile;
    /**
     ...
     */
    BlobBuffer(final ConnectionImpl connection, 
        ...
        if (length >= 0 && length <= maxMemSize) {
            ...
            if (length > 0 && in.read(buffer) != length) {
                throw new IOException("Expected to read " + 
                                length + " bytes from TDS stream");
            }
            this.length = length;
            in.close();
        } else {
            // Too big or size unknown, need to write straight to disk
     ...
     * @throws IOException if an I/O error occurs
     */
    int read(int readPtr, byte[] bytes, int offset, int len)
            throws IOException {
        // Validate parameters
        if (bytes == null) {
        ...
        public int read() throws IOException {
            ...
            if (b1 >= 0) {
                ...
                if (b2 >= 0) {
                    readPtr++;
                    if (b2 != 0
                            || b1 > 0x7F // uncomment this line for a more
                    // restrictive interpretation of 'ASCII'.
                    ) {
                        b1 = '?'; // Not ASCII set to '?'
                    }
                    return b1;
    ...
    public byte[] getBytes(long pos, int len) throws SQLException {
        ...
        try {
            // Should not do  It could cause trouble.
...
//            }
     ...
     * @throws SQLException if an error occurs
     */
    OutputStream setBinaryStream(long pos, boolean ascii)
            throws IOException {
        pos--;
        if (pos < 0) {
     ...
     * @throws SQLException if an error occurs
     */
    int setBytes(long pos, byte[] bytes, int offset, int len, boolean copy)
            throws SQLException {
        pos--;
        if (pos < 0) {
    ...
    private void writeObject(java.io.ObjectOutputStream out) throws java.io.IOException{
        ...
        // Prevent blob work file being deleted in finalize
        blobFile = null;
    }
